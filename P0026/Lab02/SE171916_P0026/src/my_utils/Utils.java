...
public class Utils {
    ...
    public static int getAvailableSeatChoice(String prompt, int min, int max, ArrayList<Passenger> listPassenger, Flight flight) {
        boolean check = true;
        int seatNumber = 0;
        Scanner sc = new Scanner(System.in);
        do {
        ...
        } while (check || seatNumber < min || seatNumber > max || isSeatOccupied(seatNumber, listPassenger, flight));
        return seatNumber;
    }

    // Check seat is occupied
    private static boolean isSeatOccupied(int seatNumber, ArrayList<Passenger> listPassenger, Flight flight) {
        for (Passenger passenger : listPassenger) {
            if (passenger.getFlight() == flight && passenger.getSeatNumber() == seatNumber) {
                return true;
            }
