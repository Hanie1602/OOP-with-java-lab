...
public class ProductManage implements IProductManage {
        ...
        switch (choice) {
            ...
            case 4:
                if (productToUpdate instanceof ProductDailyUse) {
                    ...
                    expirationDate = ((ProductDailyUse) productToUpdate).getExpirationDate();

                    Date currentExpirationDate = ((ProductDailyUse) productToUpdate).getExpirationDate();
                    while (currentExpirationDate != null && expirationDate.before(newManufacturingDate)) {
                        System.out.println("Expiration date must be after Manufacturing date!");
                        newManufacturingDate = Utils.setDate("Enter the new manufacturing date: ", "Invalid date format! Use dd/mm/yyyy", "dd/mm/yyyy");
                    }
                    ...
                    Date currentExpirationDate = ((ProductLongShelfLife) productToUpdate).getManufacturingDate();

                    while (currentExpirationDate != null && newExpirationDate.before(manufacturingDate)) {
                        System.out.println("Expiration date must be after Manufacturing date!");
                        newExpirationDate = Utils.setDate("Enter the new expiration date: ", "Invalid date format! Use dd/mm/yyyy", "dd/mm/yyyy");
                    }
                    ((ProductLongShelfLife) productToUpdate).setExpirationDate(newExpirationDate);
                }
                break;
            case 5:
                if (productToUpdate instanceof ProductDailyUse) {
                    Date newExpirationDate = Utils.setDate("Enter the new expiration date: ", "Invalid date format! Use dd/mm/yyyy", "dd/mm/yyyy");
                    manufacturingDate = ((ProductDailyUse) productToUpdate).getManufacturingDate();

                    Date currentExpirationDate = ((ProductDailyUse) productToUpdate).getExpirationDate();
                    Date minExpirationDate = Utils.addDays(currentExpirationDate, 10);
                    while (newExpirationDate.before(manufacturingDate) || newExpirationDate.after(minExpirationDate)) {
                        System.out.println("Expiration date must be after Manufacturing date!");
                        newExpirationDate = Utils.setDate("Enter the new expiration date: ", "Invalid date format! Use dd/mm/yyyy", "dd/mm/yyyy");
                    }
                    ...
                    Date newManufacturingDate = Utils.setDate("Enter the new manufacturing date: ", "Invalid date format! Use dd/mm/yyyy", "dd/mm/yyyy");
                    expirationDate = ((ProductLongShelfLife) productToUpdate).getExpirationDate();

                    while (expirationDate.before(newManufacturingDate)) {
                        System.out.println("Expiration date must be after Manufacturing date!");
                        newManufacturingDate = Utils.setDate("Enter the new manufacturing date: ", "Invalid date format! Use dd/mm/yyyy", "dd/mm/yyyy");
                    }
    ...
    public void deleteProduct() {
        ...
        if (productToDelete == null) {
            ...
            // Check if the product quantity is less than or equal to 50
            if (productToDelete.getQuantity() > 50) {
                System.err.println("Products with a quantity of more than 50 cannot be deleted.");
            } else {
